...
int config_save_network(const network_config_t *config) {
  ...
  vfs_node_t *file =
      vfs_open(CONFIG_NETWORK_FILE, VFS_O_WRONLY | VFS_O_CREAT | VFS_O_TRUNC);
  if (file == NULL) {
 ...
 */
static void build_iface_config_path(const char *iface, char *path,
                                    size_t path_size) {
...
int config_save_network_iface(const char *iface,
  ...
  /* Créer ou ouvrir le fichier */
  vfs_node_t *file =
      vfs_open(filepath, VFS_O_WRONLY | VFS_O_CREAT | VFS_O_TRUNC);
  if (file == NULL) {
    /* Essayer de créer le fichier */
...
int config_apply_network_iface(const char *iface,
                               const network_config_t *config) {
  if (iface == NULL || config == NULL) {
    return -1;
  }
...
int config_run_script(const char *path) {
  ...
  if (buffer == NULL) {
    KLOG_ERROR("CONFIG", "Failed to allocate script buffer");
    vfs_close(file);
    return -1;
  }

  int bytes = vfs_read(file, 0, 4095, buffer);
  vfs_close(file);
...
int config_load_history(char history[][256], int max_entries, int line_max) {
  ...
  /* Lire le contenu */
  uint8_t *buffer = (uint8_t *)kmalloc(8192); /* Max 8KB d'historique */
  if (buffer == NULL) {
    vfs_close(file);
    return 0;
  }

  int bytes = vfs_read(file, 0, 8191, buffer);
  vfs_close(file);
...
int config_save_history(char history[][256], int count, int start_index,
                        int line_max) {
  ...
  vfs_node_t *file =
      vfs_open(CONFIG_HISTORY_FILE, VFS_O_WRONLY | VFS_O_CREAT | VFS_O_TRUNC);
  if (file == NULL) {
  ...
  /* Construire le contenu */
  int max_size = 8192;
  uint8_t *buffer = (uint8_t *)kmalloc(max_size);
  if (buffer == NULL) {
    vfs_close(file);
    return -1;
  }
  ...
  int offset = 0;
  max_size -= 1; /* Reserve space for null terminator */
