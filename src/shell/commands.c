...
static shell_command_t commands[] = {
    ...
    {"synctest", "Test synchronization primitives (mutex, sem, etc.)",
     cmd_synctest},
...
/**
 * Commande: pci
 * Affiche la liste des périphériques PCI détectés.
 */
static int cmd_pci(int argc, char **argv) {
  (void)argc;
  (void)argv;
  ...
  while (dev != NULL) {
    /* Bus:Slot:Func */
    console_set_color(VGA_COLOR_LIGHT_CYAN, VGA_COLOR_BLACK);
    console_put_hex_byte(dev->bus);
    console_putc(':');
    console_put_hex_byte(dev->slot);
    console_putc(':');
    console_put_hex_byte(dev->func);
    console_set_color(VGA_COLOR_WHITE, VGA_COLOR_BLACK);
    ...
    /* Vendor Name */
    console_set_color(VGA_COLOR_LIGHT_GREEN, VGA_COLOR_BLACK);
    console_puts(pci_get_vendor_name(dev->vendor_id));
    console_set_color(VGA_COLOR_WHITE, VGA_COLOR_BLACK);
    ...
    /* Class */
    console_puts(pci_get_class_name(dev->class_code));

    /* IRQ */
    if (dev->interrupt_line > 0 && dev->interrupt_line < 255) {
      console_puts(" IRQ ");
      console_put_dec(dev->interrupt_line);
    }

    /* BAR0 (si présent) */
    if (dev->bar0 != 0) {
      console_puts(" BAR0=");
      console_put_hex(dev->bar0);
    }
 ...
 * Utilise thread_list_debug() qui affiche TID, état, priorité, nice, CPU time,
 * etc.
 */
static int cmd_ps(int argc, char **argv) {
  (void)argc;
...
static int cmd_netconf(int argc, char **argv) {
  ...
  if (argc < 2) {
    ...
    console_puts("  netconf eth0 static 192.168.1.100 255.255.255.0 "
                 "192.168.1.1 8.8.8.8\n");
    return 0;
  }
...
static int cmd_threads(int argc, char **argv) {
  ...
  thread_t *high = thread_create("thread_high", high_priority_task, NULL, 0,
                                 THREAD_PRIORITY_UI);
  thread_t *normal = thread_create("thread_normal", normal_priority_task, NULL,
                                   0, THREAD_PRIORITY_NORMAL);
  thread_t *low = thread_create("thread_low", low_priority_task, NULL, 0,
...
static int cmd_synctest(int argc, char **argv) {
  ...
  thread_t *m1 = thread_create("mutex_t1", mutex_test_worker, (void *)1, 0,
                               THREAD_PRIORITY_NORMAL);
  thread_t *m2 = thread_create("mutex_t2", mutex_test_worker, (void *)2, 0,
                               THREAD_PRIORITY_NORMAL);
  thread_t *m3 = thread_create("mutex_t3", mutex_test_worker, (void *)3, 0,
                               THREAD_PRIORITY_NORMAL);
  ...
  thread_t *sw1 =
      thread_create("sem_w1", sem_worker, (void *)1, 0, THREAD_PRIORITY_NORMAL);
  thread_t *sw2 =
      thread_create("sem_w2", sem_worker, (void *)2, 0, THREAD_PRIORITY_NORMAL);
  thread_t *sw3 =
      thread_create("sem_w3", sem_worker, (void *)3, 0, THREAD_PRIORITY_NORMAL);
  ...
  thread_t *w1 =
      thread_create("cv_w1", cv_waiter, (void *)1, 0, THREAD_PRIORITY_NORMAL);
  thread_t *w2 =
      thread_create("cv_w2", cv_waiter, (void *)2, 0, THREAD_PRIORITY_NORMAL);
  thread_t *w3 =
      thread_create("cv_w3", cv_waiter, (void *)3, 0, THREAD_PRIORITY_NORMAL);
  thread_t *sig =
      thread_create("cv_sig", cv_signaler, NULL, 0, THREAD_PRIORITY_HIGH);
  ...
  thread_t *r1 = thread_create("rw_r1", rwlock_reader, (void *)1, 0,
                               THREAD_PRIORITY_NORMAL);
  thread_t *r2 = thread_create("rw_r2", rwlock_reader, (void *)2, 0,
                               THREAD_PRIORITY_NORMAL);
  thread_t *r3 = thread_create("rw_r3", rwlock_reader, (void *)3, 0,
                               THREAD_PRIORITY_NORMAL);
  thread_t *ww1 = thread_create("rw_w1", rwlock_writer, (void *)1, 0,
                                THREAD_PRIORITY_NORMAL);
  thread_t *ww2 = thread_create("rw_w2", rwlock_writer, (void *)2, 0,
                                THREAD_PRIORITY_NORMAL);
...
static int cmd_schedtest(int argc, char **argv) {
  ...
  thread_t *t1 = thread_create("nice_-10", sched_test_worker_busy, (void *)1, 0,
                               THREAD_PRIORITY_UI);
  ...
  thread_t *t2 = thread_create("nice_0", sched_test_worker_busy, (void *)2, 0,
                               THREAD_PRIORITY_NORMAL);
  ...
  thread_t *t3 = thread_create("nice_+10", sched_test_worker_busy, (void *)3, 0,
                               THREAD_PRIORITY_BACKGROUND);
  ...
  thread_t *high1 = thread_create("high1", sched_test_worker_busy, (void *)10,
                                  0, THREAD_PRIORITY_HIGH);
  thread_t *high2 = thread_create("high2", sched_test_worker_busy, (void *)11,
                                  0, THREAD_PRIORITY_HIGH);

  /* Create the low priority thread that should get boosted */
  thread_t *low = thread_create("idle_boost", sched_test_worker_idle, NULL, 0,
                                THREAD_PRIORITY_IDLE);
  if (low)
    thread_set_nice(low, +19); /* Extremely low priority */
...
static int cmd_worktest(int argc, char **argv) {
  ...
  thread_t *slow_thread =
      thread_create("slow_join", (thread_entry_t)work_func_slow, NULL, 0,
                    THREAD_PRIORITY_NORMAL);
  if (!slow_thread) {
    console_set_color(VGA_COLOR_LIGHT_RED, VGA_COLOR_BLACK);
  ...
  for (int i = 0; i < 5; i++) {
    thread_t *temp =
        thread_create("temp", (thread_entry_t)work_func_increment,
                      (void *)(intptr_t)(100 + i), 0, THREAD_PRIORITY_NORMAL);
    if (temp) {
...
static int cmd_wget(int argc, char **argv) {
  if (argc < 2) {
    ...
    console_puts("\nNote: HTTPS is supported but certificate verification is "
                 "disabled.\n");
    return -1;
  }
...
static int cmd_httpd(int argc, char **argv) {
  if (argc < 2) {
    ...
    console_puts("\nCommands:\n");
    console_puts(
        "  httpd start [port]  - Start HTTP server (default port 80)\n");
