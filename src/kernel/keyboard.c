...
#include "klog.h"
#include "sync.h"
#include "syscall.h" /* EVENT_KEY constants */
#include "thread.h"
#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
 ...
 */
bool keyboard_has_char(void) { return kb_head != kb_tail; }
...
void keyboard_handler_c(void) {
  ...
  /* On envoie l'événement brute en plus de la gestion console ci-dessus */
  input_event_t ev;
  ev.type = (scancode & 0x80) ? EVENT_KEY_UP : EVENT_KEY_DOWN;
  ev.time = 0;
  ...
  if (ctrl_pressed)
    ev.data.key.flags |= 0x01;
  if (shift_pressed)
    ev.data.key.flags |= 0x02;
  if (alt_pressed)
    ev.data.key.flags |= 0x04;

  /* Tentative simple de résolution ASCII pour l'event */
  /* (C'est redondant avec switch dessus, mais plus propre pour le GUI) */
  if (ev.type == EVENT_KEY_DOWN && scancode < 128) {
    const keymap_t *km = keymap_get_current();
    if (km) {
      if (shift_pressed)
        ev.data.key.key = km->shift[scancode];
      else
        ev.data.key.key = km->normal[scancode];
    }
  }
