...
/* Helper pour accéder au framebuffer depuis les syscalls */
struct limine_framebuffer *kernel_get_primary_framebuffer(void) {
  if (g_framebuffer && g_framebuffer->framebuffer_count > 0) {
    return g_framebuffer->framebuffers[0];
  }
  return NULL;
}
...
void kmain(void) {
  ...
  /* Log memory map */
  if (g_memmap != NULL) {
    KLOG_INFO_DEC("KERNEL",
                  "Memory map entries: ", (uint32_t)g_memmap->entry_count);
    ...
    KLOG_INFO_DEC("KERNEL", "Total usable RAM (MB): ",
 ...
 */
uint64_t get_hhdm_offset(void) { return g_hhdm_offset; }
