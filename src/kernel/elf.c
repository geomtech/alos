...
static int elf_check_magic(uint8_t *e_ident) {
  return (e_ident[EI_MAG0] == ELF_MAGIC_0 && e_ident[EI_MAG1] == ELF_MAGIC_1 &&
          e_ident[EI_MAG2] == ELF_MAGIC_2 && e_ident[EI_MAG3] == ELF_MAGIC_3);
}
...
int elf_load_file(const char *filename, process_t *proc,
  ...
  for (uint16_t i = 0; i < ehdr.e_phnum; i++) {
    ...
    for (uint64_t page = 0; page < num_pages; page++) {
      ...
      if (!vmm_is_mapped_in_dir(target_dir, virt_addr)) {
        ...
        if (vmm_map_page_in_dir(target_dir, page_phys, virt_addr, page_flags) !=
            0) {
    ...
    /* Copier les données du fichier vers la mémoire */
    if (phdr->p_filesz > 0) {
/* Allouer un buffer temporaire pour lire le segment par morceaux */
#define ELF_CHUNK_SIZE 8192
      uint8_t *seg_buffer = (uint8_t *)kmalloc(ELF_CHUNK_SIZE);
      if (seg_buffer == NULL) {
      ...
      while (remaining > 0) {
        uint64_t chunk_size =
            (remaining > ELF_CHUNK_SIZE) ? ELF_CHUNK_SIZE : remaining;

        /* Lire le segment depuis le fichier */
        bytes_read =
            vfs_read(file, phdr->p_offset + offset, chunk_size, seg_buffer);
        ...
        if (vmm_copy_to_dir(target_dir, phdr->p_vaddr + offset, seg_buffer,
                            chunk_size) != 0) {
...
void elf_info(const char *filename) {
  ...
  switch (ehdr.e_type) {
  case ET_EXEC:
    console_puts("Executable\n");
    break;
  case ET_REL:
    console_puts("Relocatable\n");
    break;
  case ET_DYN:
    console_puts("Shared Object\n");
    break;
  default:
    console_puts("Other\n");
    break;
