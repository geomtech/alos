...
/* Limites de l'écran (initialisées à 0, doivent être définies par
 * mouse_set_bounds) */
...
static uint8_t g_packet_size =
    3; /* 3 pour souris standard, 4 pour souris à molette */
...
#include "input.h"
#include "syscall.h" /* EVENT_MOUSE constants */
...
static void mouse_process_packet(void) {
  ...
  if (dx != 0 || dy != 0) {
    ...
    ev.time = 0; /* TODO: timestamp */
    ev.data.mouse.x = g_mouse_state.x;
    ev.data.mouse.y = g_mouse_state.y;
    ev.data.mouse.dx = dx;
    ev.data.mouse.dy = dy;
    ev.data.mouse.buttons = g_mouse_state.buttons;
    input_push_event(&ev);
  }

  /* 2. Boutons */
  if (g_mouse_state.buttons_changed) {
    input_event_t ev;
  ...
  if (g_mouse_state.scroll != 0) {
    ...
    ev.data.mouse.y = g_mouse_state.y;
    ev.data.mouse.dx = 0;
    ev.data.mouse.dy = (int32_t)g_mouse_state.scroll;
    ev.data.mouse.buttons = g_mouse_state.buttons;
    input_push_event(&ev);
  }
}
...
const mouse_state_t *mouse_get_state(void) { return &g_mouse_state; }

void mouse_set_callback(mouse_callback_t callback) { g_callback = callback; }
...
void mouse_enable(bool enabled) { g_enabled = enabled; }

bool mouse_is_available(void) { return g_initialized && g_enabled; }
