...
void syscall_init_msr(void) {
   ...
   *              Actually: SYSRET loads CS from STAR[63:48]+16, SS from
   * STAR[63:48]+8 So for user CS=0x1B, SS=0x23: store 0x0B (0x1B-16=0x0B, but
   * with RPL...)
   *
   * Actually the correct way:
   * SYSRET: CS = STAR[63:48] + 16, SS = STAR[63:48] + 8
   * For CS=0x1B (user code 64-bit): STAR[63:48] = 0x1B - 16 = 0x0B... but
   * that's wrong
   *
