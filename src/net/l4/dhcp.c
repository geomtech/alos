 ...
 */
static void dhcp_send_raw(NetInterface *netif, uint8_t *dhcp_data,
                          int dhcp_len) {
 ...
 */
static void dhcp_parse_options(uint8_t *options, int len, uint8_t *msg_type,
                               uint32_t *server_ip, uint32_t *subnet_mask,
...
static void dhcp_handle_offer(NetInterface *netif, dhcp_header_t *dhcp,
  ...
  dhcp_parse_options(options, opt_len, &msg_type, &server_ip, &subnet_mask,
...
void dhcp_handle_packet(NetInterface *netif, uint8_t *data, int len) {
  ...
  if (cookie[0] != 0x63 || cookie[1] != 0x82 || cookie[2] != 0x53 ||
      cookie[3] != 0x63) {
    return;
  }
...
void dhcp_release(NetInterface *netif) {
  ...
  /* Envoyer un DHCPRELEASE */
  /* TODO: dhcp_send_release(netif); */
}
