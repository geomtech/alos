 ...
 *
 * NOTE: Ce wrapper utilise UNIQUEMENT le renderer bitmap SSFN pour la
 * stabilité. Le renderer scalable a été désactivé pour éviter les conflits avec
 * le nouveau système de heap kernel (spinlocks, etc.).
 *
...
int ssfn_init(void) {
  ...
  if (font_unifont_ssfn) {
    ...
    printf("[SSFN] "
           "Unifont loaded for bitmap renderer");
  } else {
    printf("[SSFN] "
           "font_unifont_ssfn is NULL!");
  }
...
int ssfn_is_initialized(void) { return ssfn_initialized; }
...
ssfn_font_t *ssfn_get_font(void) { return current_ssfn_font; }

/* Définit la couleur de premier plan */
void ssfn_set_fg(uint32_t color) { ssfn_dst.fg = color; }

/* Définit la couleur d'arrière-plan (0 = transparent) */
void ssfn_set_bg(uint32_t color) { ssfn_dst.bg = color; }
...
int ssfn_get_cursor_x(void) { return ssfn_dst.x; }

/* Obtient la position Y du curseur */
int ssfn_get_cursor_y(void) { return ssfn_dst.y; }
...
int ssfn_print(const char *str) {
  ...
  while (*s) {
    ...
    if (ret != SSFN_OK && ret != SSFN_ERR_NOGLYPH)
      break;
  }
...
int ssfn_set_size(int size) {
  ...
  printf("[SSFN] "
         "Bitmap renderer doesn't support scalable sizes");
  return -1;
}
...
int ssfn_render_text(int x, int y, uint32_t color, const char *str) {
  ...
  printf("[SSFN] "
         "Using bitmap renderer fallback");
  ssfn_dst.x = x;
  ssfn_dst.y = y;
  ssfn_dst.fg = color;
  return ssfn_print(str);
...
int ssfn_render_text_size(int x, int y, int size, uint32_t color,
                          const char *str) {
  /* Le renderer bitmap simple ignore la taille demandée */
  printf("[SSFN] "
         "Ignoring size request for bitmap renderer");
