...
typedef struct __attribute__((packed)) {
  ...
  uint16_t bcnt; /* Buffer Byte Count (two's complement, bits 0-11, bits 12-15 =
                    1) */
...
/* pci_enable_bus_mastering moved to pci.h */
...
/**
 * Poll the device for events (used when interrupts are disabled or during
 * boot).
 */
